...
def generate_report_data(
    ...
    all_events = event_store.get_events(since=start_date, until=end_date)

    # Actions are the agent actions; decisions are the human/policy resolutions
    # appended to the chain. Count one row per action, folding each action's
    # latest decision into its effective result.
    events = [e for e in all_events if e.entry_type != "decision"]
    decisions = [e for e in all_events if e.entry_type == "decision"]
    effective_result = {e.event_id: e.result for e in events}
    for d in decisions:
        if d.related_event_id in effective_result:
            effective_result[d.related_event_id] = d.result
    ...
    for e in events:
        result = effective_result.get(e.event_id, e.result)
        ...
        if result in by_agent[e.agent_id]:
            by_agent[e.agent_id][result] += 1
    ...
    # Human approvers are recorded on the appended decision events.
    for d in decisions:
        if d.authorized_by and d.authorized_by not in ("pending", "auto-policy"):
            by_authorizer[d.authorized_by] = by_authorizer.get(d.authorized_by, 0) + 1
